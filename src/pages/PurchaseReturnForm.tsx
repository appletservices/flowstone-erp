...
export default function PurchaseReturnForm() {
  ...
  // Initial Data Load (Vendors and Edit Data)
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        // Fetch existing return if in Edit Mode
        if (isEditMode && id) {
  ...
  // Fetch Inventory Items when type changes
  useEffect(() => {
  ...
  const handleItemChange = async (rowId: string, field: keyof ReturnItem, value: string) => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const url = isEditMode ? `${API_BASE_URL}/purchase/return/update/${id}` : `${API_BASE_URL}/purchase/return/store`;
        ...
        <div className="bg-card rounded-xl border border-border">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-muted/50 border-b border-border text-sm font-medium text-muted-foreground">
              ...
              <tbody className="divide-y divide-border">
                {items.map((item) => (
                  <tr key={item.id}>
                    <td className="p-4">
                      <Select 
                        disabled={!formData.inventoryType || isLoadingItems} 
                        value={item.itemId} 
