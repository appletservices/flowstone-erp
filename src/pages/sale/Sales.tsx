...
export default function Sales() {
  ...
  const handleCancelClick = (invoiceId: number) => {
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/sale/cancel/${invoiceToCancel}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        const refreshResponse = await fetch(`${API_BASE_URL}/sale/list`, {
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`,
          },
        ...
        if (refreshResponse.ok) {
          const result: ApiResponse = await refreshResponse.json();
          setSales(result.data || []);
          setTotalRecords(result.recordsTotal || 0);
        }
      } else {
        const errorData = await response.json().catch(() => ({}));
        toast.error(errorData.message || "Failed to cancel invoice.");
      }
    } catch (error) {
      ...
      <div className="bg-card rounded-xl border border-border">
        <div className="p-4 border-b border-border flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button className="gap-2" onClick={() => navigate("/sales/create")}>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="bg-card">
                            ...
                            <DropdownMenuItem onClick={() => navigate(`/sales/edit/${invoice.id}`)}>Edit Invoice</DropdownMenuItem>
                            {/* <DropdownMenuItem>Send to Customer</DropdownMenuItem>
                            <DropdownMenuItem>Record Payment</DropdownMenuItem> */}
                            <DropdownMenuItem onClick={() => window.open(`${import.meta.env.VITE_API_URL_PRINT}/print/sale/${invoice.id}`, "_blank")}>Print Invoice</DropdownMenuItem>
                            <DropdownMenuItem
