...
export default function KataeReceiveForm() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [vendorRes, productRes] = await Promise.all([
          ...
          fetch(`${url_}/inventory/type/katae`, { headers }),
        ]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${url_}/katae/receive/store`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-3xl font-bold tracking-tight">Receive Details</p>
          </div>
        </div>
      </div>
